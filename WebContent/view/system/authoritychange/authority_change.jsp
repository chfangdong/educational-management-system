...
$().ready(function(){
	...
	// 
	$("input[id=authortyId]").click(function(){
		var arr = $(this).parent().parent().parent().find("input");
		for(var i=1;i<arr.length;i++){
			if(arr[i].checked){
				arr[0].checked = true;
...
<form action="authotychange/add.do"  method="post" class="form-horizontal">
  ...
  <table class="table">
    ...
    <%
      	List<AuthorityDTO> dtolist = (List<AuthorityDTO>)request.getAttribute("dtolist"); 
     ...
     <tr>
    	<td>
        	<label class="checkbox-inline">
            	<input type="checkbox" id="all" name="authortyId" <%if(dto.getAuth().getChecked()){ out.print("checked"); } %>  value="<%=dto.getAuth().getAuthorityId() %>"><%=dto.getAuth().getAuthorityName() %>
             </label>
        </td>
    	<td>
    		<%
    			List<AuthorityInfo> authlist =dto.getList();
    		    		
    		    		 if(authlist!=null && authlist.size()>0){
    		    		  for(AuthorityInfo auth:authlist){
    		%>
	        	 <label class="checkbox-inline">
	            	<input type="checkbox" id="authortyId" <%if(auth.getChecked()){ out.print("checked"); } %>  name="authortyId" value="<%=auth.getAuthorityId()%>"><%=auth.getAuthorityName() %>
	             </label>
         <%}} %>
        </td>
    </tr> 
	<%}} %>
    	...
    	<div class="col-sm-5 col-sm-offset-4">
            ...
            <a  href="authotychange/list.do" class="btn btn-warning">返回上一级</a>
